...
/*
 * A B+ tree can be viewed as a B-tree in which each node contains only keys 
 * (not key-value pairs), and to which an additional level is added 
 * at the bottom with linked leaves.
 * 
 *  					   [ 57 . . .]
	                        |  |
	        +---------------+  +---------------------+
...
/**
 * The simple implementation of B+-Tree, reference
 * @param <T>
 * @param <V>
 */
public class BPlusTree<T extends Comparable<T>, V> {
	 ...
	 * 
	 * a) the number of children for internal node must be in scope [
	 * Math.ceil(factor/2), factor ]. b) the number of children for leaf node
	 * must be in scope [ Math.floor(factor/2), factor-1 ]
	 */
	private int factor;
	...
	/**
	 * the abstract node definition, define the operation of leaf node and
	 * internal node.
	 * 
	 * @author bo.fangbo
	 * 
	...
	abstract class Node<T extends Comparable<T>, V> {
		...
		/**
		 * if new parent node is created when insert the key-value, the created
		 * parent node is returned, in other case, this method return null.
		 * 
		 * @param key
		 * @param value
		 * @return
	...
	class InternalNode<T extends Comparable<T>, V> extends Node<T, V> {
		 ...
		 * @return
		 */
		private Node<T, V> insert(T key, Node<T, V> leftChild,
				Node<T, V> rightChild) {
			if (this.size == 0) {
				this.size++;
			...
			System.arraycopy(this.pointers, i + 1, newPointers, i + 2,
					this.size - i);

			this.size++;
			if (this.size <= MAX_CHILDREN_FOR_INTERNAL) {
				System.arraycopy(newKeys, 0, this.keys, 0, this.size);
				System.arraycopy(newPointers, 0, this.pointers, 0,
						this.size + 1);
				return null;
			}
			...
			System.arraycopy(newPointers, m + 1, newNode.pointers, 0, this.size
					- m);
			...
			return ((InternalNode<T, V>) this.parent).insert((T) newKeys[m],
					this, newNode);
		}
	}
	...
	class LeafNode<T extends Comparable<T>, V> extends Node<T, V> {
		...
		public Node<T, V> insert(T key, V value) {
			...
			return ((InternalNode<T, V>) this.parent).insert(
					(T) newNode.keys[0], this, newNode);
		}
		...
		public V get(T key) {
			...
			return middleKey.compareTo(key) == 0 ? (V) this.values[middle]
					: null;
		}
	}
	...
	public static void main(String[] args) {
		...
		System.out.println("time cost with BPlusTree: "
		...
		System.out.println("time cost with HashMap: "
				+ (System.currentTimeMillis() - start));
