 ...
 * internal nodes may be joined or split. Because a range of child nodes is permitted, 
 * B-trees do not need re-balancing as frequently as other self-balancing search trees, 
 * but may waste some space, since nodes are not entirely full. The lower and upper bounds 
 ...
 * Time Complexity:
 * 	1. Max depth of an order m B-Tree is [logn/log(m/2)]
 * 	2. At each node, O(logm) to choose branch
 * 	3. insert or delete need O(m) to fix up info in a node
 * 	Worst cases:
 * 		1. find O(logm * (logn/logm)) = O(logn)
 * 		2. insert/delete O(m/logm * logn)
 */
	...
	private static class BTreeNode<K, V> {
		...
		public SearchResult<V> searchKey(K key) {
			...
			while (low <= high) {
				...
				if (compare(entry.getKey(), key) == 0) // entrys.get(mid).getKey()
														// == key
					break;
				else if (compare(entry.getKey(), key) > 0) // entrys.get(mid).getKey()
															// > key
					high = mid - 1;
				else
		...
		/**
		 * 删除该节点中给定索引位置的子节点。 </p> 你需要自己保证给定的索引是合法的。
		 * 
		 * @param index
		 *            - 给定的索引
		 */
		...
		if (result.isExist()) {
			...
			if (node.isLeaf())
				...
				if (leftChildNode.size() >= t) {
					...
					node.insertEntry(
							leftChildNode.entryAt(leftChildNode.size() - 1),
							result.getIndex());
					// 递归删除左子节点中的最后一个项
					return delete(leftChildNode,
							leftChildNode.entryAt(leftChildNode.size() - 1)
									.getKey());
				} else {
					// 2.b 如果节点node中后于key的子节点包含至少t个关键字
					BTreeNode<K, V> rightChildNode = node.childAt(result
							.getIndex() + 1);
			...
			else // 3
			{
				...
				if (siblingNode != null) {
					if (siblingIndex < result.getIndex()) // 左兄弟节点满足条件
					{
						...
						node.insertEntry(
								siblingNode.entryAt(siblingNode.size() - 1),
								siblingIndex);
					...
					} else // 右兄弟节点满足条件
					{
						childNode.insertEntry(node.entryAt(result.getIndex()),
								childNode.size() - 1);
						node.removeEntry(result.getIndex());
						node.insertEntry(siblingNode.entryAt(0),
								result.getIndex());
				...
				} else // 3.b 如果其相邻左右节点都包含t-1个项
				{
					...
					{
						BTreeNode<K, V> rightSiblingNode = node.childAt(result
								.getIndex() + 1);
					...
					{
						BTreeNode<K, V> leftSiblingNode = node.childAt(result
								.getIndex() - 1);
						childNode.insertEntry(
								node.entryAt(result.getIndex() - 1), 0);
